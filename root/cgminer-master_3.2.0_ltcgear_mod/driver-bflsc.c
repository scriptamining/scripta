 ...
 * Total: 3452
 ...
 scrypt:
 152 + 1 + 1 + (1 + 8) * 8 + 1 per line = 227 * 20 = 4540
 
 */
 
#define BFLSC_BUFSIZ (0x2000)
...
#define MIDSTATE_BYTES 0
#define MERKLE_OFFSET 0
#define MERKLE_BYTES 76
#define BFLSC_QJOBSIZ (MIDSTATE_BYTES+MERKLE_BYTES+4+1) // adaugat target
...
struct QueueJobStructure {
	...
	uint8_t target[4];
	uint8_t endOfBlock;
	uint8_t midState[MIDSTATE_BYTES]; //l-am pus la final sa nu incurce la trimitere
};

#define QUE_RES_LINES_MIN 3
#define QUE_MIDSTATE 0
#define QUE_BLOCKDATA 0
#define QUE_NONCECOUNT 1
#define QUE_FLD_MIN 2
#define QUE_FLD_MAX 10
...
#define BFLSC_INITMODE "ZmX,1"
#define BFLSC_INITMODE_LEN (sizeof(BFLSC_INITMODE)-1)
...
#else
#define BFLSC_IDENTITY "xBEE"
#define BFLSC_BFLSC "SHA256"
#endif
...
#define BFLSC_JALAPENO "ScriptorL"
...
static bool init_mode(struct cgpu_info *bflsc, int dev)
{
	struct bflsc_info *sc_info = (struct bflsc_info *)(bflsc->device_data);
	...
	err = write_to_dev(bflsc, dev, BFLSC_INITMODE, BFLSC_INITMODE_LEN, &amount, C_REQUESTDETAILS);
	if (err < 0 || amount != BFLSC_INITMODE_LEN) {
		applog(LOG_ERR, "%s send nonce_mode (%s) send nonce_mode=1 failed (%d:%d)",
	...
	if (err < 0 || amount < 1) {
		if (err < 0) {
			applog(LOG_ERR, "%s nonce_mode_ok (%s) nonce_mode_ok return invalid/timed out (%d:%d)",
					bflsc->drv->dname, bflsc->device_path, amount, err);
		} else {
			applog(LOG_ERR, "%s nonce_mode_ok (%s) nonce_mode_ok returned nothing (%d:%d)",
					bflsc->drv->dname, bflsc->device_path, amount, err);
		}
		return false;
	}
	
	applog(LOG_DEBUG, "init_mode(%s,%s) ok", bflsc->drv->dname, bflsc->device_path);
	printf("init_mode(%s,%s) ok\n", bflsc->drv->dname, bflsc->device_path);
...
reinit:
	...
	if (unlikely(strstr(buf, BFLSC_IDENTITY))) {
		if (ident_first) {
			applog(LOG_DEBUG, "%s detect (%s) didn't recognise '%s' <> [%s] trying again ...",
				bflsc->drv->dname, devpath, buf, BFLSC_IDENTITY );
			ident_first = false;
			goto retry;
		}
...
static void process_nonces(struct cgpu_info *bflsc, int dev, char *xlink, char *data, int count, char **fields, int *nonces)
{
	...
	for (i = QUE_FLD_MIN; i < count; i++) {
		if (strlen(fields[i]) != 4) {
			tmp = str_text(data);
			applog(LOG_ERR, "%s%i:%s invalid nonce (%s) will try to process anyway (field=%d)",
					bflsc->drv->name, bflsc->device_id, xlink, tmp, i);
			free(tmp);
		}

		hex2bin((void*)&nonce, fields[i], 2);
		nonce = ((nonce >> 8) & 0xff) | ((nonce << 8) & 0xff00);
		if(nonce != 0xffff && nonce != 0x01 && nonce != 0x11 && nonce != 0x21 && nonce != 0x31 && nonce != 0x41 && nonce != 0x51 &&
			nonce != 0x61 && nonce != 0x71){
		nonce = htobe32(nonce);
		wr_lock(&(sc_info->stat_lock));
...
static bool bflsc_send_work(struct cgpu_info *bflsc, int dev, struct work *work)
{
	...
	//work->target
	//this_target = htobe32(work->target + 7 * 4);
	//memcpy(this_target, work->target + 7 * 4, 4);
	//memcpy(data.target, "\x00\x00\xff\xff", 4);
	swap256(target_swap, work->target);
	/*
	memcpy((unsigned char *) this_target, target_swap, 4);
	this_target = htobe32(this_target);
	memcpy(data.target, (unsigned char *)this_target, 4);
	*/
	data.target[0] = target_swap[3];
	data.target[1] = target_swap[2];
	data.target[2] = target_swap[1];
	data.target[3] = target_swap[0];
	applog(LOG_DEBUG, "bflsc_send_work, target=%02x%02x%02x%02x", 
		data.target[0], data.target[1], data.target[2], data.target[3]);
