...
monitor_restart_bfgminer_if_sick = True
...
def StartMonitor(client):
    ...
    while (True):
        ...
        if must_restart and monitor_restart_bfgminer_if_sick:
            print 'Restarting'
...
class BFMinerRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    def do_GET(self):
...
if __name__ == "__main__":
    ...
    client = BFgminerClient(bfgminer_host, bfgminer_port)

    if command:
        # An argument was specified, ask cgminer and exit
        result = client.command(command, parameter)
        print result if result else 'Cannot get valid response from bfgminer'
    else:
        # No argument, start the monitor and the http server
        try:
            server = SocketServer.TCPServer((monitor_http_interface, monitor_http_port), BFGMinerRequestHandler)
