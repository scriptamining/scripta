			...
			for (epnum = 0; epnum < (int)(idesc->bNumEndpoints); epnum++) {
				...
				for (epinfo = 0; epinfo < found->intinfos[ifinfo].epinfo_count; epinfo++) {
					if (!epinfos[epinfo].found) {
						applog(LOG_DEBUG, "(%d, %d): device=%u,%u,%u, wanted=%u,%u,%u", ifinfo, epinfo,
								epdesc->bmAttributes, epdesc->wMaxPacketSize, epdesc->bEndpointAddress,
								epinfos[epinfo].att, epinfos[epinfo].size, epinfos[epinfo].ep);
